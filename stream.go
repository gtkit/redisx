...
type StreamConfig struct {
	...
	// Block 是无新消息时 XREADGROUP 的阻塞等待时长（占用一个连接）。默认 5s。
	Block time.Duration
...
// AutoClaim 接管时重投），消费继续；handler panic 会被恢复，消费终止并以
// error 返回。handler 串行执行以保证消息顺序。
// ctx 取消时返回 ctx 的错误（errors.Is(err, context.Canceled) 判断优雅退出）。
//
// 持续失败的消息会堆积在 pending，请业务侧用 XPENDING 监控。
...
// handle 执行 handler：成功即 XACK；业务失败留在 pending；panic 转为终止错误。
func (s *streamConsumer) handle(ctx context.Context, m redis.XMessage) error {
	ok, panicErr := safeStreamHandle(s.handler, m)
	if panicErr != nil {
		return panicErr
	}
	if !ok {
		return nil // 业务失败：不 ACK，留在 pending 等待重投
	}
...
// safeStreamHandle 执行 handler：ok 表示业务处理成功（应当 ACK）；
// panic 被恢复并转换为 panicErr。
func safeStreamHandle(handler func(redis.XMessage) error, m redis.XMessage) (ok bool, panicErr error) {
	defer func() {
		if r := recover(); r != nil {
			ok = false
